...
#pragma warning disable 1066,1570,1573,1591
    ...
    public class Database : IDisposable, IDatabase
    {
        ...
        {
            return Single<T>(string.Format(" SELECT * FROM {0} WHERE {1}=@0", _provider.EscapeSqlIdentifier(PocoData.ForType(typeof(T), _defaultMapper).TableInfo.TableName), _provider.EscapeSqlIdentifier(PocoData.ForType(typeof(T), _defaultMapper).TableInfo.PrimaryKey)), primaryKey);
        }
        ...
        {
            return SingleOrDefault<T>(string.Format(" SELECT * FROM {0} WHERE {1}=@0", _provider.EscapeSqlIdentifier(PocoData.ForType(typeof(T), _defaultMapper).TableInfo.TableName), _provider.EscapeSqlIdentifier(PocoData.ForType(typeof(T), _defaultMapper).TableInfo.PrimaryKey)), primaryKey);
        }
        ...
        public void BulkInsert<T>(string tableName, string primaryKeyName, IEnumerable<T> pocos, IEnumerable<string> columns, int batchSize = 25)
        {
            if (string.IsNullOrEmpty(tableName))
                throw new ArgumentNullException("tableName");
           ...
           BulkInsert(tableName, primaryKeyName, autoIncrement, pocos, columns, batchSize);
        }
        ...
        public void BulkInsert<T>(string tableName, string primaryKeyName, bool autoIncrement, IEnumerable<T> pocos, IEnumerable<string> columns, int batchSize = 25)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        var names = new List<string>();
                        //var columnsDic = new Dictionary<string, PocoColumn>();
                        if (columns == null)
                        {
                            foreach (var i in pd.Columns)
                            {
                        ...
                        do
                        {
                            ...
                            foreach (var poco in pocos.Skip(count).Take(batchSize))
                            {
                                ...
                                foreach (var colname in names)
                                {
                                    ...
                                    // Don¡®t insert the primary key (except under oracle where we need bring in the next sequence value)
                                    if (autoIncrement && primaryKeyName != null && string.Compare(colname, primaryKeyName, true) == 0)
                                    {
                                        // Setup auto increment expression
                                        string autoIncExpression = _provider.GetAutoIncrementExpression(pd.TableInfo);
                                        if (autoIncExpression != null)
                                ...
                                if (indexCmd == 0)
                                {
                                    cmdText.Append(string.Format("INSERT INTO {0} ({1}) VALUES ({2})",
                                        _provider.EscapeTableName(tableName),
                                        string.Join(",", namesArray), string.Join(",", values.ToArray())));
                                }
                                else
                                {
                                    cmdText.Append(string.Format(",({0})" , string.Join(",", values.ToArray())));
                                }
        ...
        /// <summary>
        /// Bulk update multiple rows to SQL
        /// </summary>
        /// <param name="tableName">The name of the table to insert into</param>
        /// <param name="primaryKeyName">The name of the primary key column of the table</param>
        /// <param name="pocos">The POCO objects that specifies the column values to be inserted</param>
        /// <param name="batchSize">The number of POCOS to be grouped together for each database rounddtrip</param>    
        public void BulkUpdate<T>(string tableName, string primaryKeyName, IEnumerable<T> pocos, IEnumerable<string> columns, int batchSize = 25)
        {
            try
            {
                OpenSharedConnection();
                        ...
                        do
                        {
                            ...
                            foreach (var poco in pocos.Skip(count).Take(batchSize))
                            {
                                ...
                                cmdText.Append(string.Format("UPDATE {0} SET {1} WHERE {2} = {3}{4};",
                                                             _provider.EscapeTableName(tableName), sb.ToString(),
                                                             _provider.EscapeSqlIdentifier(primaryKeyName), _paramPrefix,
                                                             index++));
                            ...
                            //if (_provider.IndexOf("oracle", StringComparison.OrdinalIgnoreCase) >= 0)
                            //{
